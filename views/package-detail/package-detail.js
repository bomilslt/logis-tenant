...
Views.packageDetail = {
    ...
    renderNotFound(main, message = null) {
        main.innerHTML = `
            <div class="empty-state">
                ...
                <h3 class="empty-state-title">${I18n.t('packageDetail.not_found')}</h3>
                <p class="empty-state-text">${message || I18n.t('packageDetail.not_found_text')}</p>
                <button class="btn btn-primary" onclick="Router.navigate('/packages')">${I18n.t('packageDetail.back_to_packages')}</button>
            </div>
        `;
    },
    ...
    renderDetail(pkg) {
        ...
        const paymentStatus = amount === 0 ? '-' : (paidAmount >= amount ? I18n.t('packageDetail.paid') : (paidAmount > 0 ? `${I18n.t('packageDetail.partial')} (${this.formatMoney(paidAmount)})` : I18n.t('packageDetail.not_paid')));
        ...
        main.innerHTML = `
            <div class="package-detail-page">
                <div class="page-header">
                    <div>
                        <button class="btn btn-ghost btn-sm" onclick="Router.navigate('/packages')">
                            ${Icons.get('arrow-left', { size: 16 })} ${I18n.t('packageDetail.back')}
                        </button>
                    ...
                    <div class="header-actions">
                        <button class="btn btn-outline" onclick="Views.packageDetail.printLabel()">
                            ${Icons.get('printer', {size:16})} ${I18n.t('packageDetail.label')}
                        </button>
                        ${remaining > 0 ? `
                            <button class="btn btn-primary" onclick="Views.packageDetail.showPaymentForm()">
                                ${Icons.get('dollar-sign', {size:16})} ${I18n.t('packageDetail.pay')}
                            </button>
                        ` : ''}
                ...
                <div class="detail-grid">
                    ...
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title">${I18n.t('packageDetail.package_info')}</h3>
                        </div>
                        <div class="card-body">
                            <div class="detail-row">
                                <span class="detail-label">${I18n.t('packageDetail.description')}</span>
                                <span class="detail-value">${pkg.description || 'N/A'}</span>
                            </div>
                            <div class="detail-row">
                                <span class="detail-label">${I18n.t('packageDetail.transport')}</span>
                                <span class="detail-value">${transportLabel}</span>
                            </div>
                            <div class="detail-row">
                                <span class="detail-label">${I18n.t('packageDetail.type')}</span>
                                <span class="detail-value">${pkg.package_type || 'N/A'}</span>
                            </div>
                            <div class="detail-row">
                                <span class="detail-label">${I18n.t('packageDetail.origin')}</span>
                                <span class="detail-value">${pkg.origin?.city || 'N/A'}, ${this.getOriginLabel(pkg.origin?.country)}</span>
                            </div>
                            <div class="detail-row">
                                <span class="detail-label">${I18n.t('packageDetail.destination')}</span>
                            ...
                            <div class="detail-row">
                                <span class="detail-label">${I18n.t('packageDetail.weight_cbm')}</span>
                                <span class="detail-value">${pkg.weight ? pkg.weight + ' kg' : '-'} / ${pkg.cbm ? pkg.cbm + ' m³' : '-'}</span>
                            </div>
                            <div class="detail-row">
                                <span class="detail-label">${I18n.t('packageDetail.quantity')}</span>
                                <span class="detail-value">${pkg.quantity || 1} pcs</span>
                            </div>
                            <div class="detail-row">
                                <span class="detail-label">${I18n.t('packageDetail.declared_value')}</span>
                                <span class="detail-value">${pkg.declared_value || '-'} ${pkg.currency || ''}</span>
                            </div>
                            <div class="detail-row">
                                <span class="detail-label">${I18n.t('packageDetail.amount')}</span>
                                <span class="detail-value font-medium">${amount > 0 ? this.formatMoney(amount) : '-'}</span>
                            </div>
                            ${this.renderPaymentBreakdown(pkg)}
                            <div class="detail-row">
                                <span class="detail-label">${I18n.t('packageDetail.created_date')}</span>
                                <span class="detail-value">${pkg.created_at ? new Date(pkg.created_at).toLocaleString(I18n.locale === 'fr' ? 'fr-FR' : 'en-US') : 'N/A'}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title">${I18n.t('packageDetail.client_recipient')}</h3>
                        </div>
                        <div class="card-body">
                            <div class="detail-section">
                                <h4 class="detail-section-title">${I18n.t('packageDetail.client')}</h4>
                                <div class="detail-row">
                                    <span class="detail-label">${I18n.t('packageDetail.name')}</span>
                                    <span class="detail-value">${clientId ? `<a href="#/clients/${clientId}">${clientName}</a>` : clientName}</span>
                                </div>
                                <div class="detail-row">
                                    <span class="detail-label">${I18n.t('packageDetail.phone')}</span>
                                    <span class="detail-value">${clientPhone}</span>
                                </div>
                            </div>
                            <div class="detail-section mt-md">
                                <h4 class="detail-section-title">${I18n.t('packageDetail.recipient')}</h4>
                                <div class="detail-row">
                                    <span class="detail-label">${I18n.t('packageDetail.name')}</span>
                                ...
                                <div class="detail-row">
                                    <span class="detail-label">${I18n.t('packageDetail.pickup_point')}</span>
                ...
                <div class="card mt-md">
                    <div class="card-header">
                        <h3 class="card-title">${I18n.t('packageDetail.actions')}</h3>
                    </div>
                    <div class="card-body">
                        <div class="actions-grid">
                            <div class="action-box">
                                <h4>${I18n.t('packageDetail.update_status')}</h4>
                                <div class="form-group">
                                    <div id="status-select-container"></div>
                                </div>
                                <div class="form-group">
                                    <input type="text" id="status-location" class="form-input" placeholder="${I18n.t('packageDetail.location_optional')}">
                                </div>
                                <div class="form-group">
                                    <textarea id="status-notes" class="form-input" rows="2" placeholder="${I18n.t('packageDetail.notes')}"></textarea>
                                </div>
                                <label class="toggle-label mb-md">
                                    <input type="checkbox" id="notify-client" checked>
                                    <span>${I18n.t('packageDetail.notify_client')}</span>
                                </label>
                                <button class="btn btn-primary" id="btn-update-status">
                                    ${Icons.get('refresh', {size:16})} ${I18n.t('packageDetail.update')}
                                </button>
                            </div>
                            
                            ${pkg.status !== 'delivered' ? `
                                <div class="action-box">
                                    <h4>${I18n.t('packageDetail.mark_delivered')}</h4>
                                    <p class="text-sm text-muted mb-md">${I18n.t('packageDetail.delivery_proof')}</p>
                                    <button class="btn btn-outline" id="btn-delivery-report">
                                        ${Icons.get('check-circle', {size:16})} ${I18n.t('packageDetail.delivery_report')}
                                    </button>
                                </div>
                            ` : `
                                <div class="action-box">
                                    <h4>${I18n.t('packageDetail.delivery_confirmed')}</h4>
                                    <p class="text-sm text-muted">${I18n.t('packageDetail.delivered_on')} ${pkg.delivered_at ? new Date(pkg.delivered_at).toLocaleDateString(I18n.locale === 'fr' ? 'fr-FR' : 'en-US') : 'N/A'}</p>
                                </div>
                            `}
                        </div>
                    </div>
                ...
                <div class="card mt-md">
                    <div class="card-header">
                        <h3 class="card-title">${I18n.t('packageDetail.tracking')}</h3>
                    </div>
                    <div class="card-body">
                        <tracking-progress 
                            status="${pkg.status}" 
                ...
                <div class="card mt-md">
                    <div class="card-header">
                        <h3 class="card-title">${I18n.t('packageDetail.detailed_history')}</h3>
                    </div>
                    <div class="card-body">
                        ${history.length > 0 ? `
                                    ...
                                    ${history.map((h, i) => `
                                        <div class="timeline-item ${i === 0 ? 'active' : ''}">
                                            ...
                                            <div class="timeline-content">
                                                ...
                                                <div class="timeline-date">${new Date(h.created_at).toLocaleString(I18n.locale === 'fr' ? 'fr-FR' : 'en-US')}</div>
                        ...
                        ` : `<p class="text-muted">${I18n.t('packageDetail.no_history')}</p>`}
                    </div>
                </div>
            </div>
        `;
    ...
    attachEvents(pkg) {
        ...
        this.statusSelect = new SearchSelect({
            ...
            placeholder: I18n.t('packageDetail.select_status'),
            items: statusItems,
            onSelect: () => {}
        });
        ...
        document.getElementById('btn-update-status')?.addEventListener('click', async () => {
            ...
            if (!newStatus) {
                Toast.error(I18n.t('packageDetail.select_status'));
                return;
            }
            ...
            try {
                ...
                Toast.success(I18n.t('packageDetail.status_updated') + (notify ? ' - ' + I18n.t('packageDetail.client_notified') : ''));
                this.render(pkg.id);
            } catch (error) {
                Toast.error(error.message || I18n.t('packageDetail.update_error'));
            }
        });
    ...
    showDeliveryReportForm(pkg) {
        ...
        Modal.open({
            title: I18n.t('packageDetail.delivery_report'),
            content: `
                <p class="text-sm text-muted mb-md">${I18n.t('packageDetail.confirm_delivery_msg').replace('{tracking}', pkg.tracking_number)}</p>
                
                <div class="form-group">
                    <label class="form-label">${I18n.t('packageDetail.received_by')}</label>
                    <input type="text" id="delivery-receiver" class="form-input" placeholder="${I18n.t('packageDetail.received_by_placeholder')}" value="${recipientName}">
                </div>
                
                <div class="form-group">
                    <label class="form-label">${I18n.t('invoices.notes')}</label>
                    <textarea id="delivery-notes" class="form-input" rows="2" placeholder="${I18n.t('packageDetail.observations')}"></textarea>
                </div>
                
                <label class="toggle-label">
                    <input type="checkbox" id="delivery-notify" checked>
                    <span>${I18n.t('packageDetail.send_confirmation')}</span>
                </label>
            `,
            footer: `
                <button class="btn btn-secondary" onclick="Modal.close()">${I18n.t('cancel')}</button>
                <button class="btn btn-primary" id="btn-confirm-delivery">${I18n.t('packageDetail.confirm')}</button>
            `
        });
        ...
        document.getElementById('btn-confirm-delivery')?.addEventListener('click', async () => {
            ...
            if (!receiver) {
                Toast.error(I18n.t('packageDetail.receiver_required'));
                return;
            }
            ...
            try {
                ...
                Toast.success(I18n.t('packageDetail.delivery_success'));
                Modal.close();
                this.render(pkg.id);
            } catch (error) {
                Toast.error(error.message || I18n.t('packageDetail.delivery_error'));
            }
        });
    },
    ...
    showPaymentForm() {
        ...
        Modal.open({
            title: I18n.t('packageDetail.register_payment'),
            content: `
                <div class="payment-summary mb-md">
                    <div class="detail-row">
                        <span class="detail-label">${I18n.t('packageDetail.package_label')}</span>
                        <span class="detail-value">${tracking}</span>
                    </div>
                    <div class="detail-row">
                        <span class="detail-label">${I18n.t('packageDetail.client')}</span>
                        <span class="detail-value">${clientName}</span>
                    </div>
                    <div class="detail-row">
                        <span class="detail-label">${I18n.t('packageDetail.total_amount')}</span>
                        <span class="detail-value">${this.formatMoney(amount)}</span>
                    </div>
                    <div class="detail-row">
                        <span class="detail-label">${I18n.t('packageDetail.already_paid')}</span>
                        <span class="detail-value">${this.formatMoney(paidAmount)}</span>
                    </div>
                    <div class="detail-row">
                        <span class="detail-label">${I18n.t('packageDetail.remaining')}</span>
                ...
                <div class="form-row">
                    <div class="form-group">
                        <label class="form-label">${I18n.t('packageDetail.amount_xaf')}</label>
                        <input type="number" id="pkg-payment-amount" class="form-input" value="${remaining}" max="${remaining}">
                    </div>
                    <div class="form-group">
                        <label class="form-label">${I18n.t('packageDetail.method')}</label>
                        <div id="pkg-payment-method-container"></div>
                    </div>
                </div>
                
                <div class="form-group">
                    <label class="form-label">${I18n.t('packageDetail.reference')}</label>
                    <input type="text" id="pkg-payment-reference" class="form-input" placeholder="${I18n.t('packageDetail.ref_placeholder')}">
                </div>
            ...
            footer: `
                <button class="btn btn-secondary" onclick="Modal.close()">${I18n.t('cancel')}</button>
                <button class="btn btn-primary" id="btn-save-pkg-payment">${I18n.t('packageDetail.save')}</button>
            `
        });
        ...
        this.pkgPaymentMethodSelect = new SearchSelect({
            ...
            placeholder: I18n.t('packageDetail.method'),
            items: [
                { id: 'mobile_money', name: I18n.t('packageDetail.mobile_money') },
                { id: 'cash', name: I18n.t('packageDetail.cash') },
                { id: 'bank', name: I18n.t('packageDetail.bank') },
                { id: 'card', name: I18n.t('packageDetail.card') }
            ],
            onSelect: () => {}
        });
        ...
        document.getElementById('btn-save-pkg-payment')?.addEventListener('click', async () => {
            ...
            if (!payAmount || payAmount <= 0) { Toast.error(I18n.t('packageDetail.invalid_amount')); return; }
            if (payAmount > remaining) { Toast.error(I18n.t('packageDetail.amount_exceeds')); return; }
            if (!method) { Toast.error(I18n.t('packageDetail.select_method')); return; }
            ...
            try {
                ...
                Toast.success(I18n.t('packageDetail.payment_saved'));
                Modal.close();
                this.render(pkg.id);
            } catch (error) {
                Toast.error(error.message || I18n.t('packageDetail.payment_error'));
            }
        });
    },
            ...
            <body>
                <div class="label">
                    ...
                    <div class="label-info">
                        <div><strong>${I18n.t('packageDetail.recipient_label')}</strong> ${recipientName}</div>
                        <div><strong>${I18n.t('packageDetail.tel_label')}</strong> ${recipientPhone}</div>
                        <div><strong>${I18n.t('packageDetail.pickup_label')}</strong> ${destination}</div>
                        <div><strong>${I18n.t('packageDetail.desc_label')}</strong> ${pkg.description || 'N/A'}</div>
                        <div><strong>${I18n.t('packageDetail.weight_label')}</strong> ${pkg.weight ? pkg.weight + ' kg' : '-'}</div>
                    </div>
                    <div class="label-footer">${new Date().toLocaleDateString(I18n.locale === 'fr' ? 'fr-FR' : 'en-US')}</div>
                </div>
    ...
    renderPaymentBreakdown(pkg) {
        ...
        if (amount === 0) {
            return `
                <div class="detail-row">
                    <span class="detail-label">${I18n.t('packageDetail.payment')}</span>
        ...
        const statusLabel = isPaid ? I18n.t('packageDetail.paid') : (isPartial ? I18n.t('packageDetail.partial') : I18n.t('packageDetail.not_paid'));
        ...
        return `
            <div class="payment-breakdown" style="margin-top:8px;padding:12px;background:var(--color-gray-50, #f9fafb);border-radius:8px;border:1px solid var(--color-gray-200, #e5e7eb);">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
                    <span style="font-size:12px;font-weight:600;text-transform:uppercase;letter-spacing:0.5px;color:var(--color-gray-500, #6b7280);">${I18n.t('packageDetail.payment_status')}</span>
                ...
                <div style="display:flex;justify-content:space-between;font-size:12px;margin-bottom:4px;">
                    <span style="color:var(--color-gray-500, #6b7280);">${I18n.t('packageDetail.total')}</span>
                    <span style="font-weight:600;">${this.formatMoney(amount)}</span>
                </div>
                <div style="display:flex;justify-content:space-between;font-size:12px;margin-bottom:4px;">
                    <span style="color:var(--color-success, #22c55e);">${I18n.t('packageDetail.paid')}</span>
                ...
                ${remaining > 0 ? `
                    <div style="display:flex;justify-content:space-between;font-size:13px;padding-top:6px;border-top:1px dashed var(--color-gray-200, #e5e7eb);">
                        <span style="font-weight:700;color:var(--color-error, #ef4444);">${I18n.t('packageDetail.remaining_label')}</span>
                ...
                ` : `
                    <div style="display:flex;align-items:center;gap:6px;padding-top:6px;border-top:1px dashed var(--color-gray-200, #e5e7eb);color:var(--color-success, #22c55e);font-size:12px;font-weight:600;">
                        ${Icons.get('check-circle', {size:14})} ${I18n.t('packageDetail.fully_paid')}
                    </div>
                `}
            </div>
        `;
    ...
    getTransportLabel(mode) {
        const labels = {
            'sea': I18n.t('packageDetail.sea'),
            'air_normal': I18n.t('packageDetail.air_normal'),
            'air_express': I18n.t('packageDetail.air_express')
        };
